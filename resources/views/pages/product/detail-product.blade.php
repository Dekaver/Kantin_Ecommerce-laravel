...
<x-app-layout>
  ...
  <div class="p-2 lg:p-8 w-full block">
    <div class="w-full rounded-t bg-white dark:bg-gray-800 dark:text-white">
      ...
      <p class="text-xl py-4 px-4 font-bold">Detail product</p>
    </div>
    ...
    <div class="w-full lg:flex">
      <div class="h-60 lg:h-auto bg-contain p-5 dark:bg-gray-800 dark:text-white bg-white lg:w-64 flex-none lg:rounded-bl overflow-hidden">
        <img src="{{asset('/img/product/'.$product->product_photo)}}" class="w-full" alt="profile" srcset="">
      </div>
      <div class="bg-white dark:bg-gray-800 dark:text-white rounded-b lg:rounded-b-none lg:rounded-br lg:w-full p-4 flex flex-col justify-between leading-normal">
        <div class="mb-8">
          ...
          <div class="font-bold text-4xl mb-2">{{$product->product_name}}</div>
          <p class="text-grey-darker text-base">{{$product->product_price}}</p>
          <p class="text-grey-darker text-base mt-3">{{$product->product_stock}}</p>
        </div>
        <div class="border-2 border-b-2 border-purple-400"></div>
        <p>{{__("Seller This Product")}}</p>
        <div class="flex items-start">
          ...
          @endif
          <div class="text-sm">
            <a href="{{route('shop.show',$product->name)}}" class="leading-none font-bold">{{$product->shop_name}}</a>
